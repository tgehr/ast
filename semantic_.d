...
Expression statementSemanticImpl(IteExp ite,Scope sc,ref StmFlags flags,bool resetConst=true){
	...
	static if(language==silq){
		Dependency guardDeps;
		bool trackGuard=quantumControl&&sc.dummyWitnessActive&&!cast(Identifier)unwrap(ite.cond);
		if(trackGuard) guardDeps=sc.expandDependency(dep);
	}
	...
	static if(language==silq) if(trackGuard&&!guardDeps.isTop){
		foreach(decl;guardDeps.dependencies){
			if(sc.rnsymtab.get(decl.getId,null) !is decl){
				sc.noteReconstructed(guardDeps);
				break;
			}
		}
	}
...
MapX!(Declaration,Declaration) collectDummyResults(Scope sc, ref FixedPointIterState state, ref DummyAnalysisData data, Declaration[] candidates)in{
	...
	foreach(ref dep;context.reconstructed) resolveTransitively(dep);
...
Expression statementSemanticImpl(ForExp fe,Scope sc,ref StmFlags flags,bool resetConst=true){
	...
	if(init.length||uninit.length){
		Expression[] cont;
		static if(language==silq) cont=sc.loopContinuation.take(fe); // follows the desugared range loop
		auto s=init~fe~uninit~cont;
		auto ce=new CompoundExp(s);
		ce.loc=fe.loc;
		auto r=statementSemanticImpl(ce,sc,flags,resetConst);
		static if(language==silq) if(cont.length) sc.loopContinuation.absorbedBy=fe;
		return r;
	}
...
CompoundExp compoundExpSemantic(CompoundExp ce, Scope sc, ref StmFlags flags, Annotation restriction_=Annotation.none, bool blscope=true, bool resetConst=true){
	...
	foreach(i,ref e;ce.s){
		...
		static if(language==silq){
			auto original=e;
			bool offered=bsc.loopContinuation.offer(ce.s,i);
		}
		e=statementSemantic(e,bsc,flags,resetConst:resetConst);
		propErr(e,ce);
		static if(language==silq){
			erpAfterStatement(original,ce.s,i,bsc);
			if(offered&&bsc.loopContinuation.finish(original)){
				ce.s=ce.s[0..i+1]; // the lowered loop absorbed the rest of the block
				break;
			}
		}
...
FunctionDef functionDefSemantic(FunctionDef fd,Scope sc){
	...
	StmFlags bdyFlags;
	static if(language==silq){
		erpBeforeBody(fd); // early-return elimination, stage 1
	}
