...
string getSuffix(Expression type){
	...
	if(auto intTy=isFixedIntTy(type)){
		return intTy.isSigned ? intTy.isClassical?"S":"s" : intTy.isClassical?"U":"u";
	}
	if(auto ℤmodTy=isℤmodTy(type)){
		return ℤmodTy.isStar ? ℤmodTy.isClassical?"X":"x" : ℤmodTy.isClassical?"M":"m";
	}
	...
	void iterate(ref Expression[] body_,Expression p0,Expression p1){
		...
		if((next0||tt0)&&(next1||tt1)){
			...
			if(next0&&next1){
				...
				forBdy.loc=loc;
				auto for_=new ForExp(i0.copy(cargs),null,ForAggregate(ForRange(false,makeConst(0),null,true,len)),forBdy);
				for_.loc=loc;
				stm(body_,for_);
			}else{
				assert(!!tt0^!!tt1);
